...
#pragma omp parallel for num_threads(64) schedule(static)
  ...
  hipStream_t htod_stream;
  hipStream_t dtoh_stream;
  hipStreamCreateWithFlags(&htod_stream, hipStreamNonBlocking);
  hipStreamCreateWithFlags(&dtoh_stream, hipStreamNonBlocking);
  ...
  hipStreamDestroy(htod_stream);
  hipStreamDestroy(dtoh_stream);
  hipDeviceSynchronize();
