...
int main(int argc, char* argv[]) {
  ...
  std::vector<hipStream_t> htod_streams(max_gpu + 1);
  std::vector<hipStream_t> dtoh_streams(max_gpu + 1);
  std::vector<hipStream_t> streams(max_gpu + 1);

  for (size_t i = 0; i < gpus.size(); ++i) {
    hipSetDevice(gpus[i]);
    device_vectors_primary[gpus[i]].resize(num_bytes * 2);
    device_vectors_secondary[gpus[i]].resize(num_bytes * 2);
    hipStreamCreateWithFlags(&streams[gpus[i]], hipStreamNonBlocking);
    hipStreamCreateWithFlags(&htod_streams[gpus[i]], hipStreamNonBlocking);
    hipStreamCreateWithFlags(&dtoh_streams[gpus[i]], hipStreamNonBlocking);
...
#pragma omp parallel for
  for (size_t i = 0; i < gpus.size(); ++i) {
    hipSetDevice(gpus[i]);
    TimeDurations::Get()->Tic("host_to_device_transfer_" + std::to_string(gpus[i]));
    CheckCudaError(hipMemcpyAsync(thrust::raw_pointer_cast(device_vectors_primary[gpus[i]].data()),
                                   thrust::raw_pointer_cast(elements.data()) + (num_bytes * i), num_bytes,
                                   hipMemcpyHostToDevice, streams[gpus[i]]));
    hipStreamSynchronize(streams[gpus[i]]);
    ...
    if (execution_type == "serial") {
      ...
      for (size_t i = 1; i < gpus.size(); ++i) {
        ...
        CheckCudaError(hipMemcpyAsync(thrust::raw_pointer_cast(device_vectors_primary[gpus[i]].data()),
                                       ...
                                       hipMemcpyDeviceToDevice, streams[gpus[0]]));

        hipStreamSynchronize(streams[gpus[0]]);
...
#pragma omp parallel for
      for (size_t i = 0; i < gpus.size() / 2; ++i) {
        ...
        TimeDurations::Get()->Tic("peer_to_peer_transfer_" + std::to_string(left_device) + "-" +
                                  std::to_string(right_device));
        hipSetDevice(left_device);
        CheckCudaError(hipMemcpyAsync(thrust::raw_pointer_cast(device_vectors_secondary[left_device].data()),
                                       thrust::raw_pointer_cast(device_vectors_primary[right_device].data()), num_bytes,
                                       hipMemcpyDeviceToDevice, streams[left_device]));

        hipSetDevice(right_device);
        CheckCudaError(hipMemcpyAsync(thrust::raw_pointer_cast(device_vectors_secondary[right_device].data()),
                                       thrust::raw_pointer_cast(device_vectors_primary[left_device].data()), num_bytes,
                                       hipMemcpyDeviceToDevice, streams[right_device]));

        CheckCudaError(hipStreamSynchronize(streams[left_device]));
        CheckCudaError(hipStreamSynchronize(streams[right_device]));
...
#pragma omp parallel for
  for (size_t i = 0; i < gpus.size(); ++i) {
    hipSetDevice(gpus[i]);
    TimeDurations::Get()->Tic("device_to_host_transfer_" + std::to_string(gpus[i]));
    CheckCudaError(hipMemcpyAsync(thrust::raw_pointer_cast(elements.data()) + (num_bytes * i),
                                   thrust::raw_pointer_cast(device_vectors_secondary[gpus[i]].data()), num_bytes,
                                   hipMemcpyDeviceToHost, streams[gpus[i]]));
    hipStreamSynchronize(streams[gpus[i]]);
...
#pragma omp parallel for
  for (size_t i = 0; i < gpus.size(); ++i) {
    ...
    for (size_t j = 0; j < num_repetitions; ++j) {
      CheckCudaError(hipMemcpyAsync(thrust::raw_pointer_cast(device_vectors_primary[gpus[i]].data()),
                                     thrust::raw_pointer_cast(elements.data() + (num_bytes * i)), num_bytes,
                                     hipMemcpyHostToDevice, htod_streams[gpus[i]]));
      CheckCudaError(
          hipMemcpyAsync(thrust::raw_pointer_cast(elements.data() + (num_bytes * gpus.size()) + (num_bytes * i)),
                          thrust::raw_pointer_cast(device_vectors_primary[gpus[i]].data() + num_bytes), num_bytes,
                          hipMemcpyDeviceToHost, dtoh_streams[gpus[i]]));

      CheckCudaError(hipStreamSynchronize(htod_streams[gpus[i]]));
      CheckCudaError(hipStreamSynchronize(dtoh_streams[gpus[i]]));
    }
  ...
  for (size_t i = 0; i < gpus.size(); ++i) {
    ...
    hipStreamDestroy(streams[gpus[i]]);
    hipStreamDestroy(htod_streams[gpus[i]]);
    hipStreamDestroy(dtoh_streams[gpus[i]]);
    hipDeviceSynchronize();
  }
